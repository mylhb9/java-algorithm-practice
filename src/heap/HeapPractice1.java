...
public class HeapPractice1 {
    public ArrayList<Integer> heapArray = null;

    public HeapPractice1(Integer data) {
    ...
    public Integer pop() {
        Integer returned_data, popped_idx, left_popped_idx, right_popped_idx;
        ...
        if(this.heapArray == null) {
            ...
            while(this.moveDown(popped_idx)) {
                left_popped_idx = popped_idx * 2;
                right_popped_idx = popped_idx * 2 + 1;

                // 오른쪽 자식 노드만 없을 때
                if(right_popped_idx >= this.heapArray.size()) {
                    if(this.heapArray.get(left_popped_idx) > this.heapArray.get(popped_idx)) {
                        Collections.swap(heapArray, popped_idx, left_popped_idx);
                        popped_idx = left_popped_idx;
                    }
                // 왼쪽/오른쪽 자식 노드가 모두 있을 때
                } else {
                    if(this.heapArray.get(left_popped_idx) > this.heapArray.get(right_popped_idx)) {
                        if(this.heapArray.get(left_popped_idx) > this.heapArray.get(popped_idx)) {
                            Collections.swap(heapArray, popped_idx, left_popped_idx);
                            popped_idx = left_popped_idx;
                        }
                    } else {
                        if(this.heapArray.get(right_popped_idx) > this.heapArray.get(popped_idx)) {
                            Collections.swap(heapArray, popped_idx, right_popped_idx);
                            popped_idx = right_popped_idx;
                        }
                    }
                }
            }
    ...
    public static void main(String[] args) {
        HeapPractice1 heapTest = new HeapPractice1(15);
        heapTest.insert(10);
        heapTest.insert(8);
        heapTest.insert(5);
        heapTest.insert(4);
